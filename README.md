# Proxiome Analysis Template

A comprehensive, reproducible analysis workflow for single-cell Proximity Network Analysis (PNA) data, built with R, Quarto, and the `pixelatorR` package.

## Overview

This template provides a complete workflow for analyzing PNA data generated by the `nf-core/pixelator` pipeline. It includes:

- Quality control and filtering
- Dimensionality reduction and clustering  
- Cell type annotation (automatic and manual)
- Differential abundance, clustering, and colocalization analysis
- Publication-ready visualizations

The template is designed to be interactive—users should actively review QC thresholds, select markers, and perform manual cell type annotation based on marker expression patterns.

## Quick Start

There are three ways to use this template:

| Method | Best For | What You Get |
|--------|----------|--------------|
| **Use as GitHub Template** | Starting a new analysis project | Full repository with version control |
| **Clone Repository** | Local development with git tracking | Full repository with renv environment |
| **Download QMD Only** | Quick analysis, existing project | Just the analysis file |

### Option 1: Use as GitHub Template (Recommended)

1. Click the green **"Use this template"** button at the top of the repository
2. Create your new repository
3. Clone your new repository and follow the [Environment Setup](#environment-setup) instructions below

### Option 2: Clone the Repository

```bash
git clone https://github.com/PixelgenTechnologies/proxiome-analysis-template.git
cd proxiome-analysis-template
```

Then follow the [Environment Setup](#environment-setup) instructions below.

### Option 3: Download QMD File Only

Download [`proxiome_analysis_template.qmd`](proxiome_analysis_template.qmd) directly and place it in your project folder. You will need to [install packages manually](#manual-package-installation).

---

## Environment Setup

### Prerequisites

Before running the analysis, ensure you have the following installed:

#### 1. R (version 4.5.1 or later)

**macOS:**
```bash
# Using Homebrew
brew install r
```
Or download from [CRAN](https://cran.r-project.org/bin/macosx/)

**Windows:**
Download and run the installer from [CRAN](https://cran.r-project.org/bin/windows/base/)

**Linux (Ubuntu/Debian):**
```bash
sudo apt update
sudo apt install r-base r-base-dev
```

#### 2. RStudio (Recommended) or VS Code

**RStudio** (recommended for beginners):
- Download from [posit.co/download/rstudio-desktop](https://posit.co/download/rstudio-desktop/)

**Visual Studio Code** (alternative):
- Download from [code.visualstudio.com](https://code.visualstudio.com/)
- Install the [R extension](https://marketplace.visualstudio.com/items?itemName=REditorSupport.r)
- Install the [Quarto extension](https://marketplace.visualstudio.com/items?itemName=quarto.quarto)

#### 3. Quarto (Optional, for rendering to HTML)

Download from [quarto.org/docs/get-started](https://quarto.org/docs/get-started/)

> **Note:** Quarto is only needed if you want to render the `.qmd` file to HTML. You can run the analysis interactively in RStudio or VS Code without Quarto.

#### 4. System Dependencies (Linux Only)

On Ubuntu/Debian systems, install required system libraries:

```bash
sudo apt install cmake libglpk-dev libhdf5-dev libfreetype6-dev libpng-dev libtiff5-dev libjpeg-dev libwebp-dev
```

### Installing R Packages

#### Using renv (Recommended)

This repository includes an `renv.lock` file that captures the exact package versions used. This ensures reproducibility across different systems.

1. **Open the project** in RStudio by double-clicking `proxiome_analysis_template.Rproj`, or open the folder in VS Code

2. **Restore the environment** by running in the R console:
   ```r
   # renv should auto-activate; if not, run:
   # install.packages("renv")
   
   renv::restore()
   ```
   
   This will install all required packages with the correct versions. The first time may take 10-30 minutes.

3. **Verify installation:**
   ```r
   library(pixelatorR)
   library(Seurat)
   library(tidyverse)
   ```

#### Manual Package Installation

If you downloaded only the QMD file or prefer not to use renv:

```r
# Install BiocManager for Bioconductor packages
install.packages("BiocManager")

# Install CRAN packages
install.packages(c(
  "tidyverse", "Seurat", "harmony", "here", 
  "Matrix", "ggraph", "pls", "ggplotify"
))

# Install Bioconductor packages
BiocManager::install("ComplexHeatmap")

# Install pixelatorR from GitHub
install.packages("pak")
pak::pak("PixelgenTechnologies/pixelatorR")
```

---

## Preparing Your Data

### Required Files

You need two types of input files:

1. **PXL files** (`.pxl`) — Raw PNA data files from the `nf-core/pixelator` pipeline
2. **Metadata file** (`metadata.csv`) — Sample information and file paths

### Folder Structure

Organize your files as follows:

```
your-project/
├── data/
│   ├── metadata.csv                    # Required: sample information
│   ├── Sample1.layout.pxl              # Your PXL data files
│   ├── Sample2.layout.pxl
│   └── ...
├── results/                            # Will be created automatically
└── proxiome_analysis_template.qmd      # The analysis template
```

### Creating the Metadata File

Create a file named `metadata.csv` in the `data/` folder with the following structure:

| Column | Required | Description | Example |
|--------|----------|-------------|---------|
| `sample_id` | Yes | Short unique identifier | `S1`, `S2` |
| `sample_alias` | Yes | Descriptive sample name | `S1_resting`, `S2_PHA` |
| `file_path` | Yes | PXL filename (not full path) | `Sample1.layout.pxl` |
| `condition` | Yes | Experimental condition | `resting`, `stimulated` |
| `donor` | No | Donor/patient ID | `D1`, `D2` |
| *other columns* | No | Any additional metadata | `age`, `batch`, etc. |

**Example `metadata.csv`:**

```csv
sample_id,sample_alias,file_path,condition,donor
S1,S1_resting,Sample1_resting.layout.pxl,resting,D1
S2,S2_stimulated,Sample2_stimulated.layout.pxl,stimulated,D1
S3,S3_resting,Sample3_resting.layout.pxl,resting,D2
S4,S4_stimulated,Sample4_stimulated.layout.pxl,stimulated,D2
```

> **Important:** The `file_path` column should contain only the filename (e.g., `Sample1.layout.pxl`), not the full path. The template automatically looks for files in the `data/` folder by prepending `data/` to each filename.

---

## Running the Analysis

### Interactive Analysis (Recommended)

1. **Open the template** in RStudio or VS Code
2. **Run code chunks interactively** using:
   - RStudio: Click "Run" button or press `Ctrl/Cmd + Enter`
   - VS Code: Use the Quarto extension's run commands
3. **Review and adjust** parameters at each checkpoint:
   - QC thresholds (Section 1)
   - Marker selection (Section 1.4)
   - Cell type annotation (Section 2)

### Rendering to HTML

To generate a complete HTML report:

**From RStudio:**
- Click the "Render" button, or
- Run `quarto::quarto_render("proxiome_analysis_template.qmd")`

**From command line:**
```bash
quarto render proxiome_analysis_template.qmd
```

---

## Expected Outputs

The analysis generates organized outputs in the `results/` folder:

```
results/
├── 01_quality_control/          # QC metrics and plots
├── 02_data_processing/          # Clustering and annotation
├── 03_abundance/                # Abundance distribution analysis
├── 04_raw_proximity/            # Colocalization score analysis
└── 05_statistical_testing/      # Differential analysis results
```

All plots are saved in both PDF (publication-quality) and PNG (quick viewing) formats.

---

## Troubleshooting

### Common Issues

<details>
<summary><strong>renv::restore() fails with compilation errors</strong></summary>

This usually indicates missing system dependencies. On Linux:
```bash
sudo apt install cmake libglpk-dev libhdf5-dev libfreetype6-dev libpng-dev libtiff5-dev libjpeg-dev libwebp-dev
```

On macOS, ensure Xcode command line tools are installed:
```bash
xcode-select --install
```
</details>

<details>
<summary><strong>"Error: cannot find function" when running the template</strong></summary>

Ensure all packages are loaded. Try running the setup chunk (Section 0.1) first, or restart R and run:
```r
renv::restore()
```
</details>

<details>
<summary><strong>Memory errors with large datasets</strong></summary>

- Close other applications to free memory
- Consider processing fewer samples at once
- Use a machine with more RAM (16GB+ recommended)
</details>

<details>
<summary><strong>Plots don't display in the IDE</strong></summary>

Ensure you're running chunks interactively, not just sourcing the file. In RStudio, use `Ctrl/Cmd + Enter` to run selected code.
</details>

### Getting Help

- **pixelatorR issues:** [PixelgenTechnologies/pixelatorR](https://github.com/PixelgenTechnologies/pixelatorR/issues)
- **Template issues:** [Open an issue](https://github.com/PixelgenTechnologies/proxiome-analysis-template/issues)

---

## Documentation Feedback

The current documentation covers the core workflow but could be enhanced with:

- Video tutorials for key analysis steps
- Example datasets for testing
- FAQ section based on common user questions
- Integration guides for downstream tools

We welcome contributions and feedback to improve this template!

---

## License

This template is released under the **GNU General Public License v2**. You are free to use, study, share, and modify the template. Any distributed derivative work must also be released under the same terms. See the [`LICENSE`](LICENSE.md) file for the full license text.
